...
Q_LOGGING_CATEGORY(client_webdav_protocol, "client.webdav.protocol")

ClientWebDAVProtocol::ClientWebDAVProtocol(QObject *parent)
    : QNetworkAccessManager(parent)
    , m_rootPath()
    , m_username()
    , m_password()
    , m_baseUrl()
    , m_currentConnectionType(ClientWebDAVProtocol::HTTP)
...

{
    ...
    connect(this, &ClientWebDAVProtocol::finished, this, &ClientWebDAVProtocol::replyFinished);
    connect(this,
            &ClientWebDAVProtocol::authenticationRequired,
            this,
            &ClientWebDAVProtocol::provideAuthenication);

    qCDebug(client_webdav_protocol) << "ClientWebDAVProtocol - created";
}

ClientWebDAVProtocol::~ClientWebDAVProtocol()
{
    qCDebug(client_webdav_protocol) << "ClientWebDAVProtocol - destroyed";
}

QString ClientWebDAVProtocol::hostname() const
{
    return m_baseUrl.host();
}

int ClientWebDAVProtocol::port() const
{
    return m_baseUrl.port();
}

QString ClientWebDAVProtocol::rootPath() const
{
    return m_rootPath;
}

QString ClientWebDAVProtocol::username() const
{
    return m_username;
}

QString ClientWebDAVProtocol::password() const
{
    return m_password;
}

ClientWebDAVProtocol::WebdavConnectionType ClientWebDAVProtocol::connectionType() const
{
    return m_currentConnectionType;
}

bool ClientWebDAVProtocol::isSSL() const
{
    return (m_currentConnectionType == ClientWebDAVProtocol::HTTPS);
}

void ClientWebDAVProtocol::setConnectionSettings(const WebdavConnectionType connectionType,
...
{
    qCDebug(client_webdav_protocol) << "Set connection settings";
    ...
    case ClientWebDAVProtocol::HTTP:
        uriScheme = "http";
        break;
    case ClientWebDAVProtocol::HTTPS:
        uriScheme = "https";
        break;
    }
    ...
    if (port != 0) {
        ...
        if (!(((port == 80) && (m_currentConnectionType == ClientWebDAVProtocol::HTTP))
              || ((port == 443) && (m_currentConnectionType == ClientWebDAVProtocol::HTTPS))))
    ...
    qCInfo(client_webdav_protocol) << "Connection settings set";
}

void ClientWebDAVProtocol::acceptSslCertificate(const QString &sslCertDigestMd5,
                                                const QString &sslCertDigestSha1)
{
    m_sslCertDigestMd5 = hexToDigest(sslCertDigestMd5);
    m_sslCertDigestSha1 = hexToDigest(sslCertDigestSha1);
...
void ClientWebDAVProtocol::replyReadyRead()
{
...
void ClientWebDAVProtocol::replyFinished(QNetworkReply *reply)
{
    qCDebug(client_webdav_protocol) << "Reply finished";

    disconnect(reply, &QNetworkReply::redirectAllowed, this, &ClientWebDAVProtocol::replyReadyRead);
    disconnect(reply, &QNetworkReply::errorOccurred, this, &ClientWebDAVProtocol::replyError);
...
void ClientWebDAVProtocol::replyDeleteLater(QNetworkReply *reply)
{
...
void ClientWebDAVProtocol::replyError(QNetworkReply::NetworkError)
{
    auto reply = qobject_cast<QNetworkReply *>(QObject::sender());
    if (reply == nullptr)
        return;

    qCDebug(client_webdav_protocol)
        << "Reply error. reply->url() == " << reply->url().toString(QUrl::RemoveUserInfo);
...
void ClientWebDAVProtocol::provideAuthenication(QNetworkReply *reply, QAuthenticator *authenticator)
{
#ifdef DEBUG
    qCDebug(client_webdav_protocol) << "ClientWebDAVProtocol::authenticationRequired()";
    QVariantHash opts = authenticator->options();
    QVariant optVar;
    foreach (optVar, opts) {
        qCDebug(client_webdav_protocol)
            << "ClientWebDAVProtocol::authenticationRequired()  option == " << optVar.toString();
    }
#endif
...
void ClientWebDAVProtocol::sslErrors(QNetworkReply *reply, const QList<QSslError> &errors)
{
    qCDebug(client_webdav_protocol)
        << "SSL error. reply->url == " << reply->url().toString(QUrl::RemoveUserInfo);
...
QString ClientWebDAVProtocol::digestToHex(const QByteArray &input)
{
...
QByteArray ClientWebDAVProtocol::hexToDigest(const QString &input)
{
    QByteArray result;
    int i = 2;
...
QString ClientWebDAVProtocol::absolutePath(const QString &relPath)
{
    return QString(m_rootPath + relPath);
}

QNetworkReply *ClientWebDAVProtocol::createCustomRequest(const QString &method,
                                                         QNetworkRequest &req,
...
#ifdef DEBUG
    qCDebug(client_webdav_protocol) << " ClientWebDAVProtocol::createRequest1";
    qCDebug(client_webdav_protocol) << "   " << method << " " << req.url().toString();
    QList<QByteArray> rawHeaderList = req.rawHeaderList();
    QByteArray rawHeaderItem;
    foreach (rawHeaderItem, rawHeaderList) {
        qqCDebug(client_webdav_protocol)
            << "   " << rawHeaderItem << ": " << req.rawHeader(rawHeaderItem);
    }
#endif
...
#ifdef DEBUG
    qCDebug(client_webdav_protocol) << " ClientWebDAVProtocol::createRequest2";
    qCDebug(client_webdav_protocol) << "   " << method << " " << req.url().toString();
    QList<QByteArray> rawHeaderList = req.rawHeaderList();
    QByteArray rawHeaderItem;
    foreach (rawHeaderItem, rawHeaderList) {
...
QNetworkReply *ClientWebDAVProtocol::list(const QString &path)
{
    qCDebug(client_webdav_protocol) << "ClientWebDAVProtocol::list() path = " << path;

    return list(path, 1);
}

QNetworkReply *ClientWebDAVProtocol::list(const QString &path, int depth)
{
    ClientWebDAVProtocol::PropNames query;
    QStringList props;
...
QNetworkReply *ClientWebDAVProtocol::search(const QString &path, const QString &q)
{
...
QNetworkReply *ClientWebDAVProtocol::get(const QString &path)
{
    ...
    qCDebug(client_webdav_protocol)
        << "ClientWebDAVProtocol::get() url = " << req.url().toString(QUrl::RemoveUserInfo);
...
QNetworkReply *ClientWebDAVProtocol::get(const QString &path, QIODevice *data)
{
                                         ...
                                         quint64 fromRangeInBytes)
{
    ...
    connect(reply, &QNetworkReply::readyRead, this, &ClientWebDAVProtocol::replyReadyRead);
    connect(reply, &QNetworkReply::errorOccurred, this, &ClientWebDAVProtocol::replyError);

    return reply;
}

QNetworkReply *ClientWebDAVProtocol::put(const QString &path, QIODevice *data, const QDateTime &dt)
{
    QNetworkRequest req = buildRequest();
    ...
    qCDebug(client_webdav_protocol)
        << "ClientWebDAVProtocol::put() url = " << req.url().toString(QUrl::RemoveUserInfo)
        << " date = " << req.rawHeader("Date");

    return QNetworkAccessManager::put(req, data);
}

QNetworkReply *ClientWebDAVProtocol::put(const QString &path,
...
QNetworkReply *ClientWebDAVProtocol::propfind(const QString &path,
...
QNetworkReply *ClientWebDAVProtocol::proppatch(const QString &path,
                                               const ClientWebDAVProtocol::PropValues &props)
{
    QString query;
...
QNetworkReply *ClientWebDAVProtocol::proppatch(const QString &path, const QByteArray &query)
{
    QNetworkRequest req = buildRequest();
...
QNetworkReply *ClientWebDAVProtocol::lock(const QString &path,
                                          qint64 secs,
                                          const QString &token,
                                          WebdavLockScope scope,
                                          WebdavLockDepth depth,
                                          const QString &owner)
{
...
QNetworkReply *ClientWebDAVProtocol::unlock(const QString &path, const QString &token)
{
    QNetworkRequest req = buildRequest();
...
QNetworkReply *ClientWebDAVProtocol::mkdir(const QString &path)
{
    QNetworkRequest req = buildRequest();
...
QNetworkReply *ClientWebDAVProtocol::copy(const QString &pathFrom,
...
QNetworkReply *ClientWebDAVProtocol::move(const QString &pathFrom,
                                          const QString &pathTo,
                                          bool overwrite)
{
    QNetworkRequest req = buildRequest();
...
QNetworkReply *ClientWebDAVProtocol::remove(const QString &path)
{
    QNetworkRequest req = buildRequest();
...
QNetworkRequest ClientWebDAVProtocol::buildRequest()
{
    QNetworkRequest req;
    if (isSSL()) {
